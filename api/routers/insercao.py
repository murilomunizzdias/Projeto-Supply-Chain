    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO "SupplyChain" (
                date, hour, delay_probability, driver_behavior_score,
                fatigue_monitoring_score, route_risk_level, port_congestion_level,
                warehouse_inventory_level, iot_temperature, shipping_costs,
                risk_classification, delivery_time_deviation, disruption_likelihood_score,
                customs_clearance_time, cargo_condition_status, historical_demand,
                lead_time_days, supplier_reliability_score, weather_condition_severity,
                order_fulfillment_status, handling_equipment_availability,
                loading_unloading_time, traffic_congestion_level, eta_variation_hours,
                fuel_consumption_rate, vehicle_gps_longitude, vehicle_gps_latitude
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                %s, %s, %s, %s, %s, %s,
                %s, %s, %s, %s, %s,
        ...
        alertas = []
        if dados.delay_probability > 0.8:
            alertas.append("Atraso critico: alta probabilidade de atraso")
        if dados.driver_behavior_score < 0.5:
            alertas.append("Comportamento do motorista abaixo do esperado")
        if dados.fatigue_monitoring_score < 0.4:
            alertas.append("Motorista com fadiga critica")
        if dados.route_risk_level > 8:
            alertas.append("Rota de alto risco")
        if dados.port_congestion_level > 8:
            alertas.append("Porto congestionado")
        if dados.warehouse_inventory_level < 100:
            alertas.append("Estoque baixo no armazem")
        if dados.iot_temperature > 30 or dados.iot_temperature < -5:
            alertas.append("Temperatura da carga fora do limite")
        ...
        return {
            ...
            "alertas_disparados": alertas if alertas else ["Nenhum alerta"]
        }
